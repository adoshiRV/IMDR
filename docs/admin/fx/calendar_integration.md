...
> ℹ **Status (2026-05-13).** Schema-side: `fx.dim_currency_pair` now uses `base_currency_id` + `quote_currency_id` (FK to `dim_currency`), and currencies anchor to `dbo.dim_country`. **Phase D Step 11 is complete** — the calendar API surface now uses `(country_code, calendar_code)` signatures throughout. See [country_anchor_design.md](../admin/calendar/country_anchor_design.md).
...
for h in hits:
    print(f"{h.currency} ({h.country_code}): {h.name}")
```
