...
For schema details (tables, columns, constraints), see [`fx_ohlc_schema.md`](fx_ohlc_schema.md).
...
| `repository_ohlc.py` | Data access layer: `FXOHLCRepository` with `bulk_upsert()`, `get_last_closes_batch()`, `delete_range()` |
...
| `pipeline_ohlc.py` | `process_hour()` 7-step orchestrator (post-ingest quality checks, anomaly prescreen) + `FXOHLCPipeline` `BasePipeline` wrapper for scheduled audit trail |
...
`process_hour()` in `src/imdr/domains/fx/pipeline_ohlc.py` — the core function called by both live and historical scripts.
...
Fetch previous hour's `close_px` for every (symbol, series) in a single batched query via `FXOHLCRepository.get_last_closes_batch()`. If `|pct_change| >= threshold`, flag as anomaly. Bars are flagged but never dropped — principle: flag, don't block.
...
Implementation: `_build_quality_checks()` in `src/imdr/domains/fx/pipeline_ohlc.py`, using check classes from `src/imdr/healthchecks/quality.py`.
