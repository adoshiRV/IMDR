...
- **Daily 08:00 SGT** — via Windows Task Scheduler, orchestrated by [scripts/imdr_daily.py](../../scripts/imdr_daily.py) alongside all other Citi daily pipelines. Registered with `estimated_tags=800`. Rationale for 08:00 vs 07:00 SGT: see [citi_tag_quota.md — Daily Batch Timing](../admin/reference/citi_tag_quota.md#daily-batch-timing).
...
```bash
python -m scripts.run_pipeline fx.citi_rate \
...
The daily live script (`fx_rate_citi_live.py`) pulls a rolling `--lookback` window (default 5 business days) on each fire. If any pair returns zero rows across the full window, the run records a `coverage` error in the RunReport:

```
{len(missing_pairs)} pair(s) returned zero rows across {lookback}-day window
```

This error appears in the email subject as `ERROR` and is emitted regardless of whether the day is a holiday. If the missing pair is expected (e.g., a new pair not yet in Citi's catalog), add it to `fx_rate.bbg_only_pairs` in `fx.yml` or remove it from `fx_rate.pairs`.

**Escalation path**: `coverage` errors in the RunReport write to the JSONL run log. The retry cron at 12:00 and 18:00 SGT reads the log — if the latest run has `coverage` errors, it re-fires. If the error persists after both retries, it surfaces in the weekly health dashboard.
