...
- **Primary key**: `pk_fx_fact_fx_rate` on `id` — NONCLUSTERED per [schema_conventions.md §5.1](../admin/reference/schema_conventions.md) (clustered index is on the natural time-series access path, not the surrogate).
...
All PAGE-compressed per [schema_conventions.md §5.2](../admin/reference/schema_conventions.md).
...
```
         ...
         ┌────────────────────────────────────────────────────┐
         │               fx.fact_fx_rate                      │
         │  (pair_id, vendor_id, frequency_id, obs_ts, tenor, │
         │   obs_date, mid_rate, fwd_points)                  │
         └────────────────────────▲───────────────────────────┘
                                  │
                 ┌────────────────┴────────────────────────┐
                 │           dbo.dim_frequency             │
                 │  DAILY (Citi EOD, BBG xlsx historical)  │
                 │  SNAPSHOT (BBG intraday 6×/day)         │
                 └─────────────────────────────────────────┘
```
