...
2. **Driver-organised, not a fixed template.** Each country section is a sequence of
   **driver sections** — one per force that moved that country's week (inflation · the
   CB decision · govt-bond supply/demand · the currency · the fiscal shock · the
   equity-macro transmission …). The week's *real story* leads and takes the room it
   deserves. No rigid 1/2/3/4 per-country block.
...
- **Editorial thesis title + deck** — names the week's *one argument* in ≤6 words + a
  one-line deck. Used for the doc masthead (the global argument) and again per country.
- **Hero stat band** — a row of big numbers + one-line captions tying each to a driver.
  Universe-level band (~6–8 numbers) in the summary; a compact 3–5-number line per
  country.
- **Desk-read table** — how the houses read the *same* print/event: **Desk · Read ·
  Implication drawn**. The signature device that turns one number into the disagreement.
...
The global read a PM could stop after. In order:

1. **Masthead** — running header `RV CAPITAL · RATES & FX · WEEKLY CROSS-BANK
   SYNTHESIS`, the **editorial thesis title + deck** (the week's one *global* argument),
   the desk line (`{window} · Compiled {date}`), and a **Sources** line (houses covered
   + official docs, IMDR library).
2. **Universe hero stat band** — ~6–8 numbers with captions spanning the week's biggest
   moves across the universe (the standout yield, the standout FX, the key CB rate, the
   surprise print …). Grounded to the market / econ / CB layers.
3. **The week in brief** — the global synthesis narrative (a few tight paragraphs): the
   week's gravity, the cross-country throughline, how the big desks split, and the
   trades that express it.
4. **Cross-asset moves matrix** — one table, all countries (ordered by what moved):
   **FX vs USD (WoW %) · 2y (WoW bp) · 10y (WoW bp) · equity (WoW %) · one-line read**,
   with an oil/vol/credit footnote. "n/l" where a series isn't loaded (label swap/OIS;
   cash govt yields not loaded).
5. **Universe desk-by-desk** — **Bank · This week's flagship(s) · Cross-country core
   message** — the map of what each major house pushed this week across the region.
6. **The big cross-cutting themes** — the 4–8 strands the whole street is discussing
   (e.g. behind-the-curve BoJ · AI/semis cycle · the Fed path · EM-relief rotation),
   each *built up*: who introduced it, who corroborated with what numbers, who
   dissented, what breaks it. A `Sources:` line of report_ids ends each theme.
7. **The week's key tensions (cross-universe)** — a short judging preview: the 3–5
   sharpest live disagreements across the region and, briefly, whose logic holds
   (`Solid`/`Weak`/`Stale`) — the summary-level echo of the per-country audits below.

   *(Sections 1–7 above carry **bucket ①, the week that was** — what printed and moved.
   Sections 8–9 below carry buckets ② and ③.)*

8. **This week — estimates & where the desks split (bucket ②, the heaviest).** The
...
9. **The week ahead — preview (bucket ③, lighter, same structure).** A lighter table of
   the *following* week's setup: **Event · Date · Consensus / prior · What it tests ·
   Early lean** — plus a few tight lines. Lighter than §8, but the same discipline: no
...
- **Country header** — `{COUNTRY}` + a one-line country thesis + a compact hero-stat
  line (3–5 numbers).
- **Driver sub-sections** — ordered by what moved that country's week; each = a
  title/kicker + whichever of {desk-read table · scoreboard · chart-spec ·
  reconciliation prose · so-what callout} the driver needs, always closing on a
  **position implication**. Cover rates · FX · equities · credit + the wealth-effect
  channel where live.
- **Country trade board** — **# · House · Trade · Entry / Current · Target / Stop ·
  Status** (new / persists / target-hit / stopped / evolved) + an INTERNAL COHERENCE
  CHECK where trades interact.
...
- **Total macro calendar** — one chronological whole-universe calendar (releases + CB
  ...
  TE labelled, each line framed by what it tests.
- **Source register** — **Bank · Document · Date · IMDR folder** across all houses, plus
  an honest **"deeper reads not yet performed"** line, and the internal-use /
...
- **Hunt the per-country sell-side WEEKLIES — they are the spine of each country read.**
  Every country in the universe has at least one sell-side *weekly* flagship in the
  library; **find it and read it in full** — it is the backbone, not an optional extra.
...
- **Country-wise exhaustive hunt — FULL depth is the priority.** The weekly is the
  spine, but do not stop there: for **every country**, sweep the *entire* in-window
  corpus tagged to it — across **all houses and all note types** (weeklies, dailies,
  strategy notes, rates/FX notes, data flashes, econ comments, desk colour) — and
  **deep-read every substantive note in full**, not just the flagship. Coverage per
  country is measured by how much of its in-window flow was actually read, not by titles
  scanned. This is the single most important input; when in doubt, read more.
...
- Content over citation; every line understandable on its own; every table row explained
  ...
  number *was*.
- This is the WEEKLY spec. **Independent of the daily — do not port the daily's
  Deltas-lead, universe CB dashboard, or A/B/C/D layout here.**
