...
| `bofa` | markets.ml.com | Liferay HTML scrape across 22 hubs + portlet `pdfResourceUrl` resolver POST → direct PDF on `research1.ml.com/C?q=<token>&e=<email>&h=<hash>` (HMAC self-auth). Programmatic login (PingFederate, like Barclays). | live, ~30-50/day net est ([details](bofa.md)) |
