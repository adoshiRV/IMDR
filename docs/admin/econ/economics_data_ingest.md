...
- **Date**: 2026-06-05
- **Owner**: TBD
- **Status**: **SCHEMA APPLIED + 4 VENDORS LOADED + KOREA IN PRODUCTION** (FRED 173 + HKMA 29 + KOSIS 164 + REB 4 = **370 dim total, ~325,579 observation rows**). All loaded indicators have observations. Schema migrations 068–073 + 076 + 077 + 078 live. **Korea KOSIS + REB ingest is now automated**: 21 prod fetchers under `scripts/econ/` wire into `scripts/imdr_weekly.py` (2 weekly housing fetchers) and `scripts/imdr_monthly.py` (19 KOSIS fetchers); each fetcher auto-invokes `scripts.econ._runner.invoke_loader` on completion — no manual load step. Remaining vendors (FRED, HKMA) still use the manual `load_econ_indicator_from_playground` path. 4 more vendors have parquet on disk ready to load (Stats NZ / RBI FX / RBI Bulletin / RBA / ABS). **Korea wiring map = 16/16 covered** (14 ✅ fully + 1 ⚠ partial 4.3 + 1 parked 3.4). Coverage gap-close round added 5 new fetchers: M-aggregates (M2 + Lf), IIP + Mfg Capacity Util, Consumer Survey (CCI components), BSI (Realised + Outlook × All + Mfg), Corporate financial ratios (Mfg × all 13 ratios).
