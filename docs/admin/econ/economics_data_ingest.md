...
- **Date**: 2026-06-23 (US completeness build-out: FRED-US scheduled + BIS REER + real PCE; 193 active / ~199k obs; 3.4 ✅; score 15✅/1⚠/0❌; prior same date: US Track B + Track A PROD-LIVE; prior 2026-06-22 India Track B Phase J)
- **Owner**: TBD
- **Status**: **SCHEMA APPLIED + 4 VENDORS LOADED + KOREA + INDONESIA + INDIA + US IN PRODUCTION** (FRED 173 + HKMA 29 + KOSIS 164 + REB 4 + BIS KR 1 + ID 308 + IN ~1,242 + US 193 active = **~2,114 dim total**). All loaded indicators have observations. Schema migrations 068–073 + 076 + 077 + 078 + 102 + 103 + **105** + **106** live. **Korea KOSIS + REB ingest is now automated**: 21 prod fetchers under `scripts/econ/kr/` wire into `scripts/imdr_weekly.py` (2 weekly housing fetchers) and `scripts/imdr_monthly.py` (19 KOSIS + 1 BIS). **Indonesia**: 28 prod fetchers at `scripts/econ/id/`; `id_monthly.py` wired into `imdr_monthly.py`; BIS + SRBI also in `imdr_daily.py`. **India Track A (2026-06-19)**: 15 prod fetchers at `scripts/econ/in/`; cadence-split across **two** orchestrators — `in_daily.py` (frequency_scope=["DAILY"]) + `in_monthly.py` (frequency_scope=["MONTHLY","WEEKLY","DAILY","QUARTERLY","ANNUAL"]) — wired into `scripts/imdr_daily.py:PIPELINES` + `scripts/imdr_monthly.py:PIPELINES` 2026-06-19. (`in_quarterly.py` was created then folded into monthly on 2026-06-19; `imdr_quarterly.py` has no India entry.) rbi_bulletin.py requires headed Chrome (TSPD). Track A ops runbook: [india/india_prod_pipeline.md](india/india_prod_pipeline.md). **India Track B (NEW 2026-06-22)**: 16-stream PDF harvester (`scripts/econ/in/govt/daily_pull.py`) + disk-walk ingest (`scripts/econ/in/govt/ingest_filings.py`) writing to `research.dim_report` + Qdrant + SharePoint. `in_daily.py` rewritten to dual-track — runs Track A IMD rainfall + Track B harvest + ingest in one orchestrator. 15 streams active (cga deferred — ASP.NET PostBack); 209 official docs in `research.dim_report` (discovery backfill 2026-06-11: rbi 130 + mospi 31 + dea_in 21 + mof_in 14 + ppac 13). Track B ops runbook: [india/india_govt_prod_pipeline.md](india/india_govt_prod_pipeline.md). Execution tracker: [development/in_govt_filings.md](../development/in_govt_filings.md). **Korea cell 4.4 now has the real BOK Base Rate** (BIS.POLICY_RATE.KR). **US Track A (2026-06-23)**: 15 prod fetchers at `scripts/econ/us/`; two orchestrators (`us_monthly` + `us_daily`) **wired into `scripts/imdr_monthly.py:PIPELINES` + `scripts/imdr_daily.py:PIPELINES` 2026-06-23 (PROD-LIVE)**; G.6 code-review passed; `us_daily` ran end-to-end rc=0 on first scheduled run. Ops: [united_states/united_states_prod_pipeline.md](united_states/united_states_prod_pipeline.md). **Completeness build-out 2026-06-23**: (1) FRED-US scheduled fetchers — `fred_us_daily` (56 series, DAILY+WEEKLY, in `us_daily`) + `fred_us_monthly` (52 series, in `us_monthly`); `seed_us.yml` generated from DB active-set — the 26 migration-106-deactivated source-dup series excluded (verified still inactive post-reload); library `src/imdr/domains/econ/fred_http.py` (`FredClient`); adds Philly Fed + Dallas Fed; daily FRED-US history to 2010+ (~165k obs); cross-country OECD mirror stays in playground. (2) BIS `bis_us.py` (WS_EER NEER+REER, in `us_monthly`) — closes cell 3.4 ✅. (3) Real PCE (T20806/DPCERX) added to `bea_personal_income.py` — closes 1.1 consumption quantity. **193 active indicators / ~198,775 obs. Score: 15 ✅ / 1 ⚠ / 0 ❌.** Remaining vendors (HKMA) still use the manual `load_econ_indicator_from_playground` path. **US Track B (2026-06-23)**: 11-stream Fed/Treasury/NY Fed document ingest promoted to `scripts/econ/us/govt/`; `us_daily.py` rewritten to dual-track (Track A EIA+Treasury Debt + Track B `ingest_filings --since-days 7`); migration 107 applied (seeds `nyfed` vendor); **145 reports / 2,320 chunks** in `research.dim_report` + Qdrant + SharePoint (backfill 2024-07→2026-06; fed 125/2,192 · treasury_us 12/28 · nyfed 8/100); **wired into `imdr_daily.py:PIPELINES` 2026-06-23** via the `us_daily` dual-track entry. `imdr_daily.py` must run under conda `imdr` env (Python 3.11) — tiktoken/qdrant_client required. Track B ops: [united_states/united_states_govt_prod_pipeline.md](united_states/united_states_govt_prod_pipeline.md). Execution tracker: [development/us_govt_filings.md](../development/us_govt_filings.md). **Korea wiring map = 16/16 covered** (14 ✅ fully + 1 ⚠ partial 4.3 + 1 parked 3.4).
- **AU update (2026-07-14)**: housing + labour tracking buildout — Cotality monthly HVI, SQM Research (new vendor), APRA MADIS, SEEK (new vendor), ANZ-Indeed, plus ABS labour age/state breakdowns. AU total now **758 indicators / 523,708 obs** (DB-verified 2026-07-14). See §2.1 below and [australia/australia_indicator_inventory.md](australia/australia_indicator_inventory.md).
...
| **India — IMD + BIS + FAO + RBI + MOSPI + DPIIT + CGA + DGCIS + UPAg** | **PROD (Track A)**: `scripts/econ/in/{imd,bis,fao,rbi,mospi,dpiit,cga,dgcis,upag}/` × 15 fetchers. **Two** cadence-split orchestrators: `scripts/econ/in/in_daily.py` (frequency_scope=["DAILY"]) + `scripts/econ/in/in_monthly.py` (frequency_scope=["MONTHLY","WEEKLY","DAILY","QUARTERLY","ANNUAL"]). (`in_quarterly.py` was created then folded into monthly 2026-06-19; `imdr_quarterly.py` has no India entry.) **Wired into `scripts/imdr_daily.py:PIPELINES` + `scripts/imdr_monthly.py:PIPELINES` 2026-06-19.** Playground originals at `playground/econ/in/{vendor}/` preserved as legacy sandbox. Library: `src/imdr/domains/econ/mospi.py` + `src/imdr/domains/econ/upag.py`. Migration 103 seeded `upag` vendor in `dbo.dim_vendor` (migration 089 had omitted it — blocker resolved). **PROD (Track B — NEW 2026-06-22)**: 16-stream PDF harvester (`scripts/econ/in/govt/daily_pull.py`) + ingest (`scripts/econ/in/govt/ingest_filings.py`) writing to `research.dim_report` + Qdrant + SharePoint. `in_daily.py` rewritten to a dual-track orchestrator: Track A IMD rainfall + Track B harvest + `ingest_filings --since-days 2`. 15 of 16 streams active (cga press notes deferred — ASP.NET PostBack). 209 docs in `research.dim_report` (backfill 2026-06-11: rbi 130 · mospi 31 · dea_in 21 · mof_in 14 · ppac 13). Migrations 086 + 089 applied. Track B ops: [india/india_govt_prod_pipeline.md](india/india_govt_prod_pipeline.md). | ✅ **~1,242 indicators × ~99,810 obs** (DB-verified 2026-06-19; incl. DGCIS 198/31,086 + UPAg AIAPY 324/15,030 + UPAg MSP 28/353 + UPAg IMC 16/128 + RBI Bulletin 23 tables ~478/~1,188 + pre-existing BIS/FRED/DBIE 26/39,569) | ✅ — **auto-load via `in_daily` / `in_monthly`** | Production 2026-06-19 (Track A). Track B production 2026-06-22. No keys required (MOSPI/DPIIT/CGA/DGCIS/UPAg/FAO/BIS all public; RBI DBIE uses static auth headers). **rbi_bulletin.py requires headed Chrome (TSPD) — monthly orchestrator must run on a display-capable host.** FCNR/NRI T34 live (T34 NRI Deposits: FCNR(B)/NR(E)RA/NRO outstanding+flow, 8 ind). Track A ops: [india/india_prod_pipeline.md](india/india_prod_pipeline.md). Track B ops: [india/india_govt_prod_pipeline.md](india/india_govt_prod_pipeline.md). |

| **US — BLS + BEA + Census + Treasury + EIA + BIS + FRED-US** | **PROD-LIVE (Track A)**: `scripts/econ/us/{bls,bea,census,treasury,eia,bis,fred}/` × 18 fetchers. Two cadence-split orchestrators: `scripts/econ/us/us_monthly.py` (BLS ×5 + BEA ×4 + Census ×3 + Treasury MTS + BIS ×1 + FRED-US monthly ×1) + `scripts/econ/us/us_daily.py` (EIA energy + Treasury Debt + FRED-US daily + Track B filings). **Wired into `scripts/imdr_monthly.py:PIPELINES` + `scripts/imdr_daily.py:PIPELINES` 2026-06-23.** Library connectors: `src/imdr/domains/econ/{bls,bea,census,treasury_fiscaldata,eia,fred}_http.py`. G.6 code-review gate passed (0 blockers). `us_daily` ran end-to-end rc=0 on first scheduled run. **Completeness build-out 2026-06-23**: (a) FRED-US scheduled via `fred_us_daily` (56 series, DAILY+WEEKLY) + `fred_us_monthly` (52 series); `seed_us.yml` generated from DB active-set, excluding the 26 migration-106-deactivated source-dup series (verified still inactive); adds Philly Fed + Dallas Fed; library `fred_http.py` (`FredClient`); daily history to 2010+ (~165k obs). (b) BIS `bis_us.py` (WS_EER NEER+REER, no key; in `us_monthly`) — closes cell 3.4 ✅. (c) Real PCE T20806/DPCERX added to `bea_personal_income.py` — closes 1.1 consumption quantity. Migrations 105 + 106 applied. **PROD-LIVE (Track B)**: 11-stream Fed/Treasury/NY Fed document ingest at `scripts/econ/us/govt/`. `us_daily.py` dual-track: Track A EIA+Treasury Debt+FRED-US daily + Track B `ingest_filings --since-days 7`. Migration 107 applied (seeds `nyfed` official_cb vendor). **145 reports / 2,320 chunks** LIVE in `research.dim_report` + Qdrant + SharePoint (backfill 2024-07→2026-06). Wired via `us_daily` entry in `imdr_daily.py:PIPELINES` 2026-06-23. Track B ops: [united_states/united_states_govt_prod_pipeline.md](united_states/united_states_govt_prod_pipeline.md). Execution tracker: [development/us_govt_filings.md](../development/us_govt_filings.md). | ✅ **193 active indicators × ~198,775 obs** (FRED-US 106+ · BEA 36 · BLS 29 · Census 10 · EIA 3 · Treasury 4 · BIS 2 + Philly/Dallas adds; 26 exact-dup FRED rows deactivated by migration 106; score 15✅/1⚠/0❌) | ✅ **auto-load via `us_monthly` / `us_daily`** — wired 2026-06-23 | Production 2026-06-23. Keys: `IMDR_ECON_BLS_KEY`, `IMDR_ECON_BEA_KEY`, `IMDR_ECON_CENSUS_KEY`, `IMDR_ECON_EIA_KEY`, `IMDR_ECON_FRED_KEY` (+`_KEY2` rotation); Treasury + BIS are keyless. Track B: no keys — all US Fed/Treasury/NY Fed streams are public. **conda `imdr` env required** — `imdr_daily.py` task must run under Python 3.11 (tiktoken + qdrant_client); `sys.executable` binds subprocesses automatically. Ops: [united_states/united_states_prod_pipeline.md](united_states/united_states_prod_pipeline.md). |
...
**Status 2026-07-14:** Track A (data series) DB-LIVE — **758 indicators / 523,708 obs** (DB-verified 2026-07-14, up from 464/397,118 on 2026-06-11), 16 of 16 wiring-map cells ✅. `scripts/econ/au/{abs,rba,aofm,cotality}/` are built and wired into `au_monthly`/`au_daily` (no dedicated `australia_prod_pipeline.md` exists — see `australia_indicator_inventory.md` instead). **2026-07-14 housing + labour tracking buildout**: 5 new sources — Cotality monthly HVI (+10), SQM Research rents+vacancy (+33, new vendor, migration 109), APRA MADIS by-bank housing loans (+8), SEEK Job-Ad/Salary indices (+90, new vendor, migration 109), ANZ-Indeed Job Ads (+3) — plus ABS labour age/state breakdowns (LF/LF_UNDER group 9→84 indicators). New unit `aud_pw` via migration 110. All wired into `au_daily` (SQM) / `au_monthly` (the other four); not yet in top-level `imdr_daily.py`/`imdr_monthly.py` — separate sign-off gate, unchanged. See [`australia/australia_indicator_inventory.md`](australia/australia_indicator_inventory.md). Track B (govt filings) **Phase J PROD-BUILT 2026-06-11** — `scripts/econ/au/govt/` + `scripts/econ/au/au_daily.py` in place; 8 official streams writing to `research.dim_report` + Qdrant + SharePoint; 9 reports / 201 chunks live. Final gate: `scripts/imdr_daily.py:PIPELINES` registration pending OK. See [`australia/australia_govt_prod_pipeline.md`](australia/australia_govt_prod_pipeline.md).
