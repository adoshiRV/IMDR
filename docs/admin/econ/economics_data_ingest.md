...
- **Date**: 2026-06-16 (BIS KR policy-rate fetcher; see below for earlier history)
- **Owner**: TBD
- **Status**: **SCHEMA APPLIED + 4 VENDORS LOADED + KOREA IN PRODUCTION** (FRED 173 + HKMA 29 + KOSIS 164 + REB 4 + BIS KR 1 = **371 dim total**). All loaded indicators have observations. Schema migrations 068–073 + 076 + 077 + 078 + 102 live. **Korea KOSIS + REB ingest is now automated**: 21 prod fetchers under `scripts/econ/` wire into `scripts/imdr_weekly.py` (2 weekly housing fetchers) and `scripts/imdr_monthly.py` (19 KOSIS + 1 BIS); each fetcher auto-invokes `scripts.econ._runner.invoke_loader` on completion — no manual load step. **BIS KR CBPOL** (`scripts/econ/kr/bis/bis_korea.py`) added 2026-06-16 — wired into `imdr_daily.py` + `kr_monthly.py`; migration 102 deactivated `FRED.RATES.KR_DISCOUNT.KR` (BOK discount rate, was incorrectly treated as policy-rate proxy). **Korea cell 4.4 now has the real BOK Base Rate**. Remaining vendors (FRED, HKMA) still use the manual `load_econ_indicator_from_playground` path. **Korea wiring map = 16/16 covered** (14 ✅ fully + 1 ⚠ partial 4.3 + 1 parked 3.4).
