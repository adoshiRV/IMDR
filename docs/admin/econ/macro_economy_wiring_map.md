...
Updated 2026-06-23: PROD-LIVE — wired into `imdr_monthly.py:PIPELINES` + `imdr_daily.py:PIPELINES`. Completeness build-out (FRED-US scheduled + BIS REER + real PCE) completed 2026-06-23.

| Engine | A | B | C | D |
|---|:---:|:---:|:---:|:---:|
| **Growth** | ✅ Private *(Census MARTS retail + BEA Personal Income/real PCE — T20806/DPCERX added 2026-06-23, chained 2017 $mn)* | ✅ Fiscal *(Treasury MTS receipts/outlays/deficit + Debt-to-Penny daily)* | ✅ External *(Census FT-900 goods+services trade + BEA ITA net exports)* | ✅ Macro Core *(BEA NIPA GDP adv/2nd/3rd + BLS payrolls/unemp/LFPR + FRED GDPNow/INDPRO/CFNAI)* |
| **Inflation** | ✅ Input Costs *(BLS import/export price indexes + EIA WTI/Brent/Henry Hub daily)* | ✅ Producer *(BLS PPI final demand + stage-of-processing)* | ✅ Domestic *(BLS ECI total comp + JOLTS quits/openings + productivity + AHE)* | ✅ CPI *(BLS CPI-U headline+core+components + BEA PCE price)* |
| **External** | ✅ Terms of Trade *(BLS export/import price ratio — closes former ❌)* | ✅ Current Acc *(BEA ITA BalCurrAcct + Gds/Serv/PrimInc/SecInc decomp, quarterly)* | ✅ Capital Acc *(BEA ITA financial account DI/PI/Other/Reserves + BEA IIP net stock; TIC stock deferred)* | ✅ FX/REER *(BIS NEER+REER broad — WS_EER M.{N,R}.B.US via `scripts/econ/us/bis/bis_us.py`; latest REER 107.26 / NEER 101.69; in `us_monthly` — 2026-06-23)* |
| **Policy** | ⚠️ Demand Trans *(FRED SLOOS, mortgage rates, BUSLOANS — FRED is source-clean; no upstream source upgrade exists)* | ✅ Balance Sheets *(FRED Z.1 TDSP/CMDEBT + Treasury Debt-to-Penny daily)* | ✅ Fin Conditions *(UST curve + IG/HY/BAA OAS + NFCI + VIX — FRED)* | ✅ Policy *(Fed funds, EFFR, SOFR, IORB, Fed BS, RRP — FRED; SEP dot-plot via Track B)* |

US score: **15 ✅ / 1 ⚠️ / 0 ❌**. Remaining ⚠: 4.1 Demand Trans (FRED is source-clean; SLOOS/H.8/mortgage rates — no source upgrade exists; cosmetic).

> **Tier-1 source-agency discovery COMPLETE + DATA LOADED 2026-06-22.** Migration 105 registered vendors bls/bea/census/treasury_us; **82 indicators / 30,563 obs loaded** into `econ.fact_indicator` (BEA 36 back to 1947, BLS 29, EIA 3, Treasury 4, Census 10) via the user-supervised one-shot loader. FRED is a mirror; the source agencies have working playground fetchers, all producing loader-valid parquet. Built: **BLS** (CPI · PPI · Employment Situation · ECI/JOLTS/productivity · import/export prices → cells 1.4·2.1·2.2·2.3·2.4·**3.1**), **BEA** (GDP/NIPA · Personal Income/PCE · ITA current+financial account · IIP → 1.1·1.3·1.4·2.4·**3.2**·**3.3**), **Census** (MARTS retail · FT-900 trade · New Residential Construction → 1.1·1.3), **Treasury** (MTS fiscal · Debt-to-Penny → 1.2·4.2), **EIA** (WTI/Brent/Henry Hub spot → 2.1). Identity checks pass: BoP CA decomposition diff=0, export/import ToT=1.12, fiscal receipts−outlays=deficit diff=0. Migration 106 applied 2026-06-23: 26 exact-dup FRED rows deactivated; BLS price indices recategorised `other → cpi`.
>
> **PROD-LIVE 2026-06-23.** 15 fetchers (BLS ×5 + BEA ×4 + Census ×3 + Treasury ×2 + EIA ×1) + 5 connector modules at `src/imdr/domains/econ/` + 2 orchestrators (`us_monthly` + `us_daily`) wired into `scripts/imdr_monthly.py:PIPELINES` + `scripts/imdr_daily.py:PIPELINES` 2026-06-23. 68 unit tests passing; G.6 code-review gate passed (0 blockers). `us_daily` ran end-to-end rc=0 on first scheduled run. Plan: [united_states/us_coverage_plan.md](united_states/us_coverage_plan.md). Ops ref: [united_states/united_states_prod_pipeline.md](united_states/united_states_prod_pipeline.md). Track B (FOMC docs) also PROD-LIVE 2026-06-23 — [united_states/united_states_govt_prod_pipeline.md](united_states/united_states_govt_prod_pipeline.md).
>
> **Completeness build-out 2026-06-23.** (1) **FRED-US promoted to scheduled fetchers** — new library `src/imdr/domains/econ/fred_http.py` (`FredClient`; key via `get_settings().econ_fred_key` + numbered-sibling env rotation); new prod fetchers `scripts/econ/us/fred/fred_us_daily.py` (DAILY+WEEKLY, 56 series) + `fred_us_monthly.py` (MONTHLY+QUARTERLY+ANNUAL, 52 series) + shared seed `fred_us_daily.py:_fred_seed.py` (108 curated ACTIVE US FRED series); `seed_us.yml` generated from the DB active-set — excludes the 26 migration-106-deactivated source-dup series (verified: 26 still inactive post-reload) and carries post-106 categories. Plus 2 macro-PM additions: Philly Fed (`GACDFSA066MSFRBPHI`) + Dallas Fed (`BACTSAMFRBDAL`) regional manufacturing surveys. FRED-US is now on a scheduled refresh: `fred_us_daily` in `us_daily`, `fred_us_monthly` in `us_monthly`; daily series history deepened to 2010+ (~165k obs). The cross-country OECD mirror (non-US FRED) stays in playground. New unit test `tests/unit/test_econ/test_fred_http.py`. (2) **BIS US NEER/REER** — `scripts/econ/us/bis/bis_us.py` (WS_EER `M.{N,R}.B.US`; vendor `bis`; in `us_monthly`) — registers the prod fetcher for cell 3.4, flipping ⚠️ → ✅. Latest REER 107.26 / NEER 101.69. (3) **BEA real PCE** — added T20806/DPCERX (Real Personal Consumption Expenditures, chained 2017 $mn, monthly) to `bea_personal_income.py` — fills the 1.1 consumption quantity leg. Latest ~$16.8tn. Hard code review passed (`is_active` landmine confirmed avoided; `fred_http` aligned to `get_settings()`). **US now 193 active indicators / ~198,775 obs.** Score: **15 ✅ / 1 ⚠️ / 0 ❌**.
