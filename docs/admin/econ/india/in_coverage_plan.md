...
- [⚠] **A13 (partial discovery)** DGCIS trade scrape — site mapped 2026-06-10. Four sub-portals at `tradestat.commerce.gov.in`: EIDB (annual), **MEIDB (monthly — our target)**, FTPA (performance analysis), FTSPCC (principal commodity). MEIDB uses Laravel CSRF-protected forms (`POST /meidb/commoditywise_{export,import}`) — form-field names decoded but POST flow + Excel download deferred to next session. Fallback path: PIB monthly trade press releases (stable HTML at `pib.gov.in/PressReleseDetailm.aspx`). See [`playground/econ/dgcis/discovery/findings.md`](../../../playground/econ/dgcis/discovery/findings.md).
