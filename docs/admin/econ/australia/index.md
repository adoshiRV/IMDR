...
- [`au_cb_documents.md`](au_cb_documents.md) — download checklist for RBA / AOFM / Treasury / APRA / ABS document-style sources (Board minutes, SMP, FSR, Budget Papers, etc.). Document pipeline (not data pipeline). **Tier-1 fully LIVE 2026-06-10** at [`playground/econ/au/govt/`](../../../playground/econ/au/govt/) — 6 fetchers, ~33 items captured: Governor's Statement + Board Minutes + SMP + FSR (Playwright/Akamai) + Treasury publications + APRA quarterly stats (plain httpx). AOFM remains corp-firewall-blocked (host-specific). Manifest-only — no DB writes until research-doc pipeline absorbs filings.
