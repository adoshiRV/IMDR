...
AU macroeconomic data. **Status: DB-LIVE (manual load) — 463 indicators / 397,053 obs. ABS 17 fetchers / 20 dataflows (178 indicators) + RBA 9 fetchers via CSV snapshot (119 indicators, incl. TIB + I2 ICP + F15 REER + F17 zero-coupon curve) + AOFM 5 fetchers (157 indicators) + Cotality (6 daily HVI) + FRED OECD mirror (3 indicators). 15 of 16 wiring-map cells ✅. Second-most-populated country after Indonesia.**
...
- [`au_cb_documents.md`](au_cb_documents.md) — download checklist for RBA / AOFM / Treasury / APRA / ABS document-style sources (Board minutes, SMP, FSR, Budget Papers, etc.). Document pipeline (not data pipeline). **Tier-1 fully LIVE 2026-06-10** at [`playground/econ/au/govt/`](../../../playground/econ/au/govt/) — **8 fetchers, 53 items captured**: Governor's Statement + Board Minutes + SMP + FSR + filtered Speeches (Playwright/Akamai) + Treasury publications + APRA quarterly stats + ABS release commentary (plain httpx). Only AOFM remains Tier-3 (corp-firewall-blocked, host-specific). Manifest-only — no DB writes until research-doc pipeline absorbs filings.
