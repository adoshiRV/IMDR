...
Last updated: 2026-07-14 (AU: housing + labour tracking buildout — Cotality monthly/SQM/APRA MADIS/SEEK/ANZ-Indeed + ABS labour age/state breakdowns, 758 indicators DB-verified; prior 2026-06-19 IN Track A)
...
| **Australia (AU)** | DB-LIVE (manual load) — **758 indicators / 523,708 obs** (DB-verified 2026-07-14); ABS 20 fetchers / 22 dataflows (324 indicators incl. derived ToT + CPI_Q + **new labour age/state breakdowns**) + RBA 9 fetchers via CSV snapshot (119 indicators incl. TIB + ICP + REER + ZCY) + AOFM 5 fetchers (157) + Cotality (16 — 6 daily + **10 monthly HVI, NEW**) + **SQM Research (33, rents+vacancy, NEW) + APRA MADIS (8, by-bank housing loans, NEW) + SEEK (90, job-ad+salary, NEW) + ANZ-Indeed (3, job ads, NEW)** + FRED-mirror (3); **16 of 16 cells ✅** (3.1 ToT closed 2026-06-11). Phase G blocker lifted. Production promotion pending user sign-off. | [australia/](australia/) | [§7.7](macro_economy_wiring_map.md#77-australia-au) | [australia_indicator_inventory.md](australia/australia_indicator_inventory.md) |
| **New Zealand (NZ)** | **PROD-LIVE 2026-06-18** — Stats NZ Track A, ~1,105 indicators across 14 fetchers (release-CSV: CPI/GDP/BoP; Infoshare: PPI/CGPI/OTI/HLPI/LCI/QES/ECT/OMT/RTS/HLFS + **CPI core**), wired into `imdr_monthly.py`. **CPI core (42 series — 27 exclusion + 15 trimmed-mean/percentile, 1988-Q4→) loaded 2026-06-19**; `statsnz_cpi_core.py` not yet in `nz_monthly.py` (gated). RBNZ Track B planned. Ops: [new_zealand_prod_pipeline.md](new_zealand/new_zealand_prod_pipeline.md) | [new_zealand/](new_zealand/) | [§7.8](macro_economy_wiring_map.md#78-new-zealand-nz) | [new_zealand_indicator_inventory.md](new_zealand/new_zealand_indicator_inventory.md) |
| **India (IN)** | **PROD-LIVE 2026-06-19** — Track A Phase G complete. **15 prod fetchers** at `scripts/econ/in/{vendor}/`; **two** cadence-split orchestrators wired into `imdr_daily.py` + `imdr_monthly.py`. (`in_quarterly.py` folded into monthly 2026-06-19; `imdr_quarterly.py` has no India entry.) ~**1,242** indicators / ~**99,810** obs (DB-verified 2026-06-19) across IMD · BIS · FAO · RBI FX Reserves · RBI Key Rates · MOSPI CPI/IIP/NAS GDP · DPIIT WPI/8-Core · CGA · DGCIS · UPAg IMC/MSP/AIAPY · **RBI Bulletin 23 tables incl. T34 NRI Deposits + T40 BoP 151 indicators**. rbi_bulletin.py requires headed Chrome (TSPD). FCNR/NRI live. Migration 103 seeded `upag` vendor. Ops: [india_prod_pipeline.md](india/india_prod_pipeline.md). 14 of 16 wiring-map cells covered (8 ✅ + 6 ⚠ + 2 ❌). | [india/](india/) | [§7.12](macro_economy_wiring_map.md#712-india-in) | — |
