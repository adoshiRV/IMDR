...
ID macroeconomic data. **Status: 250 indicators × 26,757 observations live in `econ.fact_indicator` as of 2026-06-09.** Split across three vendors: BPS (`vendor_code='bps'`, 82 indicators) + BI (`vendor_code='bi'`, 162 indicators across SEKI tables, Survey publications, and bank rates) + BIS (`vendor_code='bis'`, 6 indicators). **All 16 wiring-map cells covered; 13 of 16 are full ✅** — only 2.1 Input Costs (one full source — could derive more), 3.1 Terms of Trade (NBToT derivable in analytics from existing BPS export/import price indices), and 3.4 FX/REER strengthening (BIS reserves composition pending) remain ⚠ partial. **Prod-live 2026-06-09**: 25 fetchers live under `scripts/econ/{bps,bi,bis}/`, orchestrator at `scripts/econ/id/id_monthly.py`; wired into `scripts/imdr_monthly.py:PIPELINES` 2026-06-09. `scripts.econ.bis.bis_indonesia` is also registered in `scripts/imdr_daily.py:PIPELINES` for same-day policy-rate capture.
...
| **BI SEKI** (Statistik Ekonomi Keuangan Indonesia) — `bi.go.id/SEKI/tabel/` | None | Slow (XLSX) | Money, fiscal, credit, SBN, BoP, reserves, external debt | **Live** (2026-06-08) — 9 SEKI tables scraped: Money Supply (I.1), Monetary Base (I.2), Bank BS (I.3), Bank Credit (I.4), Fiscal Revenue/Spending/Financing (IV.1/2/3), SBN Position (IV.4), BoP (V.1), FX Reserves (V.9), External Debt SULNI (VI.1). **71 indicators × 8,151 obs.** Legacy multi-period sheets, deeper BoP sub-tables (V.2-V.8) deferred to Phase D4 |
