...
`dim_curve` is auto-seeded on each pipeline run via `bulk_seed_from_universe()` — no manual step required. The pipeline's `transform()` calls the seeder idempotently before resolving `curve_id` lookups.
...
| `transform_skipped_unmapped_curves` | Curve not in dim_curve | Re-run the pipeline — `dim_curve` auto-seeds on each run via `bulk_seed_from_universe()` |
...
### Lookback window + coverage error escalation

`rates_citi_live.py` pulls a rolling `--lookback` window (default 5 business days). If any **active** curve returns zero rows across the full window, the run records a `coverage` error in the RunReport:

```
{len(missing_curves)} active curve(s) returned zero rows across {lookback}-day window
```

This appears in the email as `ERROR`. Ceased curves (e.g. LIBOR after 2023-06) are excluded from the coverage check — only `curve_status='active'` or `'reformed'` curves trip the gate.

**Escalation path**: the same `coverage` error is also emitted by the hourly pipeline (`rates_citi_live_hourly.py`) where it promotes to a `coverage_gap` warning when the missing curve's market is classified as `open` or `post_close` at run time. The retry cron (12:00 and 18:00 SGT) re-fires if the daily run logged a `coverage` error.

### `TagQuotaExceeded`

Exits with code 1. The retry cron handles re-fire automatically. If quota is exhausted repeatedly, check `data/cache/citi_tag_quota.json` for cumulative usage and consider whether a stale `--no-cache` run recently consumed an outsized batch. The hourly pipeline uses a separate quota file (`data/cache/citi_tag_quota_hourly.json`) and separate OAuth client, so it does not share this budget.
