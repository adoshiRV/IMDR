...
> ℹ **Status (2026-05-13).** Schema-side: all `rates.*` dim tables now FK to `dbo.dim_country` (`country_id`), not `calendar.dim_market`. **Phase D Step 11 is complete** — the calendar API surface now uses `(country_code, calendar_code)` signatures. The old `market_code`-only signatures have been removed. See [country_anchor_design.md](../admin/calendar/country_anchor_design.md).
...
with Session(connector.engine) as session:
    decisions = rate_decisions(session, country_code="US")
    for d in decisions:
