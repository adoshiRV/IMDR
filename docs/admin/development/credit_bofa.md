...
## 7. Changelog
- **2026-07-17 (f)** — **UBS + HSBC covered-bond fix + DRY consolidation.** Added shared `canonical.looks_like_credit()` / `CREDIT_TITLE_RE` (covered bonds, CLO/ABS/CMBS/RMBS/securitized, HY/HG/IG, CDS/CDX/iTraxx, JACI/JULI/EMBI/CEMBI, lev-loans). UBS (`classifiers/ubs.py`) + HSBC (`classifiers/hsbc.py`) now upgrade a desk-coded RATES/"" note to CREDIT when the title names a credit product (e.g. UBS "Global Rates Comment: Covered Bonds…", HSBC "Covered Bond Insight"); never touches EQUITY/FX/MACRO. Nomura repointed to the shared helper (removed its local regex). `test_credit_title_override.py` (15) + full research suite green. Credit now covered across every vendor that publishes it.
- **2026-07-17 (e)** — **Credit coverage audit + Nomura fix.** DB check (last 4d portal): credit flows from jpm 106 / barclays 62 / citi 16 / goldman 10 / db 7 / ms 5 / bofa 5 / stanc / socgen. Six research vendors showed 0 credit; title audit found one **material gap: Nomura's CLO / Securitized Products / ABS stream was mis-tagged RATES** (asset_class_id=FI + empty assetClasses → `_split_fi` RATES default). Fixed: `classifiers/nomura.py` `_split_fi` now consults the title (`_CREDIT_TITLE_RE`: clo/cdo/abs/cmbs/rmbs/mbs/securitiz/covered-bond/HY/HG/IG/cds/cdx/itraxx/jaci/lev-loan/corp-credit) before defaulting to RATES. +`test_nomura_classifier.py` (9 tests). Minor residuals (documented, not chased): HSBC/UBS "Covered Bond" notes tagged RATES (1 each); ANZ/Westpac/BNP genuinely publish ~no corporate credit via scraped scope (correct 0). UBS IG/HY credit may sit in an un-scraped Neo hub — discovery-scope probe deferred.
