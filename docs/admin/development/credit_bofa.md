...
- [ ] **1b** Per-vendor classifier credit-path probe (UBS, Nomura, Citi, BNP, ANZ, Westpac) + fixes
- [ ] **1c** JPM `isResearch=N` credit-desk bypass (extend `MACRO_DESK_KEEP`)
- [ ] **2a** BofA MFA login build (needs live login w/ user for OTP sample) → uncomment `ingest_today.py:119,163` → docs LIVE
