...
**Running dev-doc.** Started 2026-07-16. Owner: Arjun. Status: **active / investigation done, Fold 1 not yet shipped.**
...
- [~] **2a** BofA portal — **MFA login SOLVED & verified 2026-07-17.** Second factor = **email security token** ("Restricted Access Authentication"): 8-digit token from `bofamarkets@bofa.com`, subject "BofA Mercury Portal Token", body "Your token is: NNNNNNNN", 5-min expiry, filed in Outlook `Research\BOFA`. `login_bofa.py` now: detects the token page → `_read_portal_token()` polls `Research\BOFA` via win32com (`_TOKEN_RE`, freshest <4min) → `_submit_token()` fills+submits. E2E live test: `LOGIN OK; is_authenticated=True; Home - BofA Markets`. **Remaining (needs decisions):** (i) prod-wiring — uncomment `ingest_today.py:119,163` [NEEDS explicit OK]; (ii) BofA's built-in single-name **discovery** drop (`filters/bofa.py` series-regex: Kosmos Energy Ltd etc.) must be relaxed for the credit hubs or it undoes the single-name-credit goal; (iii) hub crawler vs firehose vs both (run BofA ONCE per cycle so two logins don't race for the token).
