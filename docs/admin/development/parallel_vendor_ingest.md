...
- Path-collision check: if `target.exists()` and `size > 0`, re-hash; bytes match -> idempotent return; bytes differ -> `PdfPathCollisionError`. Reject the v1 plan of erroring on any collision — bytes-match is just an idempotent retry and must succeed.
