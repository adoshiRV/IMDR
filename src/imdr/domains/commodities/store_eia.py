"""Parquet store for EIA petroleum status data.

Layout: data/parquet/commodities/fact_eia/{YYYY-MM}.parquet
"""
...
_log = structlog.get_logger("CmdtyEIAParquetStore")

DATA_ROOT = Path("data/parquet/commodities/fact_eia")
NATURAL_KEY = ["series_name", "region", "obs_date"]
...
def write(
    ...
    """Write EIA DataFrame to parquet, split by month."""
    ...
    for month, group in df.groupby("_month"):
        ...
        write_df = group[["obs_date", "series_name", "region", "value"]].copy()
