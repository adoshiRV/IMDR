"""Parquet store for equity index levels.

Layout: data/parquet/equities/fact_index_level/{YYYY-MM}.parquet
"""
...
_log = structlog.get_logger("EquityIndexParquetStore")

DATA_ROOT = Path("data/parquet/equities/fact_index_level")
...
def write(
    ...
    """Write index level DataFrame to parquet, split by month."""
    ...
    for month, group in df.groupby("_month"):
        ...
        write_df = group[["obs_date", "ticker", "value"]].copy()
