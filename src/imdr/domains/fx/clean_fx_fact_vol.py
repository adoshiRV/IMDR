...
class HardBoundViolationRule(CleaningRule):
    ...
    def detect(
        ...
        for (strike, vol_type), (lo, hi) in self._ranges.items():
            when_clauses.append(
                f"([strike] = '{strike}' AND [vol_type] = '{vol_type}' "
                f"AND ([value] < {lo} OR [value] > {hi}))"
            )
        filter_expr = " OR ".join(when_clauses)

        sql = f"""
            SELECT [id], [obs_date], [pair_id], [strike], [tenor],
                   [vol_type], [value]
            FROM {table}
    ...
    def describe(self, row: pd.Series) -> str:
        ...
        return (
            f"hard bound: pair_id={row['pair_id']} {row['strike']} {row['tenor']} "
...
class RobustOutlierRule(CleaningRule):
    ...
    def detect(
        ...
        sql = f"""
            ...
            WHERE 1=1
              {where}
            ORDER BY [pair_id], [strike], [tenor], [obs_date]
        """
    ...
    def describe(self, row: pd.Series) -> str:
        return (
            f"robust outlier: pair_id={row['pair_id']} {row['strike']} {row['tenor']} "
...
class PercentageChangeRule(CleaningRule):
        ...
        sql = f"""
            WITH with_prev AS (
                SELECT [id], [obs_date], [pair_id], [strike], [tenor],
                       [vol_type], [value],
                       LAG([value]) OVER (
                           PARTITION BY [pair_id], [strike], [tenor], [vol_type]
                           ORDER BY [obs_date]
                       ) AS prev_val
                FROM {table}
                WHERE 1=1
    ...
    def describe(self, row: pd.Series) -> str:
        ...
        return (
            f"pct_change: pair_id={row['pair_id']} {row['strike']} {row['tenor']}"
