"""Equity Citi Velocity Historical Backfiller.

Supports both sub-products: INDEX (24 global indices), VIX (5 vol indices).
Modes: range, catchup, gaps.

Edit the variables below and run:
    python -m scripts.equity.citi.equity_citi_historical
"""
...
GAPS_FILE = "data/gaps/equity_gaps.txt"
...
_TABLE_MAP = {"index": "fact_index_level", "vix": "fact_vix"}
...
def _build_pipeline(product: str, connector, settings, universe, start, end):
    ...
    if product == "index":
        ...
        raise ValueError(f"Unknown product: {product}. Use: index, vix")


def _run_pipeline(connector, settings, universe, start, end, label):
    log.info("processing", label=label, start=str(start.date()), end=str(end.date()))
    pipeline = _build_pipeline(PRODUCT, connector, settings, universe, start, end)
    rows = pipeline.run()
    return rows
...
def main() -> int:
    ...
    report = RunReport(pipeline_name=f"equity.{PRODUCT}_citi_historical")
    ...
    try:
        ...
        if settings.run_log_dir:
            ...
            table_name = _TABLE_MAP.get(PRODUCT, f"fact_{PRODUCT}")
            log_path = (
                Path(settings.run_log_dir)
                / "equities"
                / table_name
                / f"equity_{PRODUCT}_citi_historical_{ts:%Y%m%d_%H%M%S}.jsonl"
            )
