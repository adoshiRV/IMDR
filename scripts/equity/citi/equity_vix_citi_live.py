"""Equity VIX Family Citi Velocity Daily EOD Runner.

Target table: [equities].[fact_vix]
Schedule: Daily (via imdr_daily.py)
Source: Citi Velocity Historical Data API

Usage:
    python -m scripts.equity.citi.equity_vix_citi_live
    python -m scripts.equity.citi.equity_vix_citi_live --date 2026-03-25
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="VIX Family Daily EOD Ingest")
...
def main() -> int:
    ...
    report = RunReport(pipeline_name="equity.vix_citi_live")
    ...
    if args.date:
        ...
        # Equity/VIX intent suggests "NY" (NYSE); follow-up tracked in
    ...
    log.info("equity_vix_live_start", date=str(target.date()))

    connector = MSSQLConnector(settings)
    try:
        t0 = time.perf_counter()
        pipeline = EquityVixPipeline(
        ...
        if settings.run_log_dir:
            log_path = (
                ...
                / "equities" / "fact_vix"
                / f"equity_vix_citi_live_{target:%Y%m%d}.jsonl"
            )
            report.flush_jsonl(log_path)

        log.info("equity_vix_live_complete", date=str(target.date()),
        ...
        log.exception("equity_vix_live_failed")
        report.error("pipeline", "Daily VIX family ingest failed")
        report.finish()
        return 1
    finally:
        connector.dispose()
...
def _send_report_email(
    ...
    """Build and send the equity VIX ingest report email."""
    formatter = EquityIngestFormatter()
    has_errors = report.has_errors

    subject = formatter.format_subject(
        pipeline_name="equity.vix_citi_live",
        run_date=target,
        rows_loaded=result,
