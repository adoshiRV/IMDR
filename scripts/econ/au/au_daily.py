...
sub-orchestrator that produces AU data at daily cadence. Currently:

  - scripts.econ.au.govt.ingest_filings  — discovers + ingests AU govt
    policy filings (RBA, Treasury, APRA, ABS, Westpac-MI, NAB) into
    ``research.dim_report`` + ``research.fact_chunk`` + Qdrant +
    SharePoint via ``imdr.research.filings.ingest_filing``.

Future daily components (high-frequency rates, AUD spot, Cotality daily
HVI) can be added to ``PIPELINES`` below.

Distinct from the ``_country_runner`` pattern used by weekly/monthly:
that wires `econ.fact_indicator` indicator-row counts. This orchestrator
queries ``research.dim_report`` for the filings-side summary.
...
def _render_email(
    *,
    ...
    subject = (
        f"[IMDR Daily AU] {status} — {n_new} new filings, {n_chunks} chunks "
...
<h3>IMDR AU Daily — government filings ingest</h3>
