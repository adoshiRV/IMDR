...
Runs every fetcher / sub-orchestrator that produces IN data at daily cadence:
  ...
  Track B — govt/CB filings:
    ...
    - scripts.econ.in.govt.ingest_filings --since-days 2
                                          (walk recent date-folders → research.dim_report
                                           + research.fact_chunk + Qdrant + SharePoint)
...
3-pipeline design lets a harvester failure leave ingest unblocked — each
subprocess is isolated; the orchestrator continues regardless of rc and reports
all outcomes in the email.

Wired into ``scripts/imdr_daily.py:PIPELINES`` (already registered).
...
# Force UTF-8 stdout — Devanagari / em-dashes / bullet chars come through subprocess output.
...
# ============================================================================
# REGISTERED PIPELINES — extend as new IN daily fetchers land
# ============================================================================
...
def _engine():
    """Engine for post-run snapshots. Uses ODBC Driver 18 because
...
def _filings_snapshot(run_started_at: datetime.datetime) -> dict:
    """Pull stats on IN official filings ingested at/after run_started_at.

    Mirrors kr_daily._filings_snapshot — scopes to official_* vendors for
    country IN.
    """
    eng = _engine()
    try:
            ...
            by_vendor = conn.execute(
                text(
                    ...
                    WHERE v.vendor_category LIKE 'official_%'
                      AND r.country_id = (SELECT id FROM dbo.dim_country WHERE country_code = 'IN')
                      AND r.created_at >= :t0
...
<table width='100%' cellpadding='0' cellspacing='0' style='background-color:#0d2137;'>
  <tr>
    <td style='padding:18px 24px;'>
      <span style='color:#ffffff;font-size:20px;font-weight:bold;'>IMDR &mdash; India Econ Ingest (Daily)</span>
...
<table width='96%' cellpadding='6' cellspacing='0' style='margin:8px auto 0 auto;border-collapse:collapse;border:1px solid #ddd;'>
  <tr style='background:#f5f5f5;'><td style='border:1px solid #ddd;font-weight:bold;width:200px;'>Orchestrator</td><td style='border:1px solid #ddd;font-family:Consolas,monospace;'>scripts.econ.in.in_daily</td></tr>
...
<table width='100%' cellpadding='0' cellspacing='0' style='margin-top:24px;background-color:#f0f0f0;border-top:2px solid #ddd;'>
  <tr><td style='padding:12px 24px;color:#888;font-size:12px;'>
    Generated by IMDR | scripts.econ.in.in_daily | {run_completed_at:%Y-%m-%d %H:%M:%S} UTC |
...
def main(argv: list[str] | None = None) -> int:
    ...
    for cmd in PIPELINES:
        # derive a display name: last non-flag token in cmd
        name = next(
            (tok for tok in reversed(cmd) if not tok.startswith("-")),
            cmd[-1],
        )
    ...
    # --- Snapshots: Track A (DAILY indicators) + Track B (filings) ---
    ...
    if (
        ...
        try:
            ...
            log.exception("in_daily_email_failed")
            print(f"\n!! email render/send failed:\n{traceback.format_exc()}")
    elif args.no_email:
        print("\n(email skipped by --no-email)")
    else:
        log.info("email_disabled_skipping_in_daily_summary")
