...
class TestIsHoliday:
    def test_us_new_years(self):
        assert is_holiday("US", date(2026, 1, 1)) is True

    def test_us_regular_day(self):
        assert is_holiday("US", date(2026, 3, 23)) is False

    def test_eu_target2_new_years(self):
        assert is_holiday("EU", date(2026, 1, 1)) is True

    def test_eu_target2_christmas(self):
        assert is_holiday("EU", date(2026, 12, 25)) is True

    def test_jp_holiday(self):
        # Jan 1 is a public holiday in Japan
        assert is_holiday("JP", date(2026, 1, 1)) is True
...
class TestIsTradingDay:
    def test_regular_monday(self):
        assert is_trading_day("US", date(2026, 3, 23)) is True

    def test_saturday(self):
        assert is_trading_day("US", date(2026, 3, 21)) is False

    def test_us_holiday(self):
        assert is_trading_day("US", date(2026, 1, 1)) is False

    def test_israel_sunday_is_trading(self):
        """Sunday is a trading day in Israel."""
        assert is_trading_day("IL", date(2026, 3, 22)) is True

    def test_israel_friday_not_trading(self):
        """Friday is not a trading day in Israel."""
        assert is_trading_day("IL", date(2026, 3, 20)) is False
...
class TestIsMarketOpen:
    ...
    def test_jp_during_lunch_break(self):
        ...
        assert is_market_open("JP", dt) is False
    ...
    def test_jp_during_morning_session(self):
        ...
        assert is_market_open("JP", dt) is True
...
class TestLastTradingDay:
    def test_skips_weekend(self):
        ...
        result = last_trading_day("US", before=date(2026, 3, 23))
        assert result == date(2026, 3, 20)  # Friday

    def test_skips_holiday(self):
        # Day after New Year's
        result = last_trading_day("US", before=date(2026, 1, 2))
        assert result == date(2025, 12, 31)

    def test_skips_weekend_and_holiday(self):
        # After a holiday weekend
        result = last_trading_day("US", before=date(2026, 7, 6))
...
class TestNextTradingDay:
    def test_skips_weekend(self):
        # Friday -> Monday
        result = next_trading_day("US", after=date(2026, 3, 20))
        assert result == date(2026, 3, 23)

    def test_skips_holiday(self):
        # Dec 31 -> Jan 2 (Jan 1 is holiday)
        result = next_trading_day("US", after=date(2025, 12, 31))
...
class TestTradingDaysBetween:
    def test_full_week(self):
        ...
        days = trading_days_between("US", date(2026, 3, 16), date(2026, 3, 20))
        assert len(days) == 5

    def test_includes_boundaries(self):
        days = trading_days_between("US", date(2026, 3, 23), date(2026, 3, 23))
        assert len(days) == 1
        assert days[0] == date(2026, 3, 23)

    def test_weekend_excluded(self):
        # Sat-Sun
        days = trading_days_between("US", date(2026, 3, 21), date(2026, 3, 22))
        assert len(days) == 0


class TestLastBusinessDay:
    def test_returns_datetime(self):
        result = last_business_day("US")
