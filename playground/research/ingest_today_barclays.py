"""Discover and ingest today's Barclays Live research reports.

Logs into Barclays Live programmatically using ``IMDR_BARCLAYS_USERNAME``
/ ``IMDR_BARCLAYS_PASSWORD`` from .env (no MFA — risk-based auth lets
device-trusted creds through), scrapes the ``/BU/`` home for the day's
publication tiles, resolves each ``pubId`` to a PDF ``docId`` via the
``/RSX/content-archive/v1/REST/publication/search`` JSON endpoint, and
ingests each PDF end-to-end.
...
Usage
-----
    C:/Users/adoshi/.conda/envs/imdr/python.exe playground/research/ingest_today_barclays.py
"""
...
from ingest.crawler_barclays import ReportRef, discover_reports, fetch_pdfs  # noqa: E402
...
def _research_engine(settings):
    ...
    return create_engine(
        url, pool_size=4, max_overflow=4, pool_pre_ping=True, pool_timeout=60,
...
async def _amain() -> None:
    ...
    try:
        async for ref, pdf_bytes in fetch_pdfs(PROFILE_DIR, refs):
            if pdf_bytes is None:
                outcomes.append(Outcome(
                    ref=ref, error=RuntimeError("PDF fetch returned no bytes"),
                ))
                continue
            outcomes.append(await _ingest_one_ref(
                ref, pdf_bytes, sem=sem, api_keys=api_keys, engine=engine,
    ...
    for o in outcomes:
        if o.error is not None:
            failed += 1
            print(f"  [FAIL]   {o.ref.uuid[:10]}  {o.ref.title[:40]:<40}  {type(o.error).__name__}: {o.error}")
            continue
        r = o.result
        if r.was_inserted:
            inserted += 1
            print(f"  [INS]    id={r.report_id:<5} {o.ref.uuid[:10]}  chunks={r.n_chunks:>3} embeds={r.n_embeddings:>3}  total={r.timings_s.get('total', 0):.1f}s  {o.ref.title[:40]}")
        else:
            skipped += 1
            print(f"  [DUP]    id={r.report_id:<5} {o.ref.uuid[:10]}  already in DB  {o.ref.title[:40]}")
    print("-" * 60)
