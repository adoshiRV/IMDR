"""Discover and ingest today's BNP Paribas Markets360 research reports.

Mirrors ``ingest_today_ms.py`` — same orchestration shape (discovery
→ relevance filter → ingest_one fan-out → result summary). Per-vendor
crawler lives in ``ingest/crawler_bnp.py``; per-vendor filter in
``ingest/filters/bnp.py``; per-vendor classifier in
``ingest/classifiers/bnp.py``.

Defaults
--------
* Date window: ``today and previous 3 days`` (UTC).
...
from ingest.crawler_bnp import ReportRef, discover_reports  # noqa: E402
...
def _research_engine(settings):
    ...
    return create_engine(
        url,
        pool_size=4, max_overflow=4, pool_pre_ping=True, pool_timeout=60,
        echo=False, fast_executemany=True,
        connect_args={"timeout": 60},
    )


def _read_date_env(name):
...
async def _ingest_one_ref(
    ref, *, sem, api_keys, engine, do_embed, embed_model, qdrant_writer
):
    async with sem:
        meta = ReportMeta(
...
async def _amain():
    ...
    # SGT-anchored: BNP pubDate is UTC; UTC "today" lags SGT for ops.
    ...
    if not PROFILE_DIR.exists():
        raise SystemExit(f"Profile not found: {PROFILE_DIR}\nRun explore_bnp.py first.")

    refs = await discover_reports(PROFILE_DIR, since=since, until=until)
    ...
    for r in refs:
        ...
        print(f"    {r.publish_date}  {r.uuid:>7}  {ptype:<22}  {title}")
    print()
    ...
    try:
        outcomes = await asyncio.gather(*(
            _ingest_one_ref(
                r, sem=sem, api_keys=api_keys, engine=engine,
                do_embed=do_embed, embed_model=embed_model,
                qdrant_writer=qdrant_writer,
            )
            for r in refs
        ))
    ...
    for o in outcomes:
        if o.error is not None:
            failed += 1
            print(f"  [FAIL]   {o.ref.uuid:>7}  {o.ref.title[:40]:<40}  {type(o.error).__name__}: {o.error}")
            continue
        r = o.result
        if r.was_inserted:
            inserted += 1
            print(f"  [INS]    id={r.report_id:<5} {o.ref.uuid:>7}  chunks={r.n_chunks:>3} embeds={r.n_embeddings:>3}  total={r.timings_s.get('total', 0):.1f}s  {o.ref.title[:40]}")
        else:
            skipped += 1
            print(f"  [DUP]    id={r.report_id:<5} {o.ref.uuid:>7}  already in DB  {o.ref.title[:40]}")
    print("-" * 60)
