"""Discover and ingest today's Goldman Sachs research reports.

Walks GS Marquee hub pages with the persistent profile, dedupes the
``/content/research/en/reports/...`` links by UUID, filters by
publish_date, and ingests each PDF end-to-end.

Defaults
--------
* Date window: ``today and yesterday`` (UTC). Goldman publishes from
  several time zones so "today" alone misses early-morning UK output;
  the 2-day window catches it without much noise.
* Embed: OFF (set ``IMDR_RESEARCH_EMBED=true`` in env to enable). Lets
  us iterate on the crawler without paying Voyage cost.
* Concurrency: 1 PDF at a time (sequential). Each ``ingest_one`` launches
  its own Playwright Chrome against the shared persistent profile, and
  Chrome locks the profile directory to a single process — concurrent
  fetches against the same profile fail. Real concurrency requires
  refactoring fetch.py to share one Playwright context across fetches;
  scheduled as a future improvement.

Usage
-----
    C:/Users/adoshi/.conda/envs/imdr/python.exe playground/research/ingest_today.py

Optional env
------------
    IMDR_RESEARCH_EMBED=true    # enable Voyage embedding
    IMDR_RESEARCH_SINCE=2026-05-05   # earliest publish date (YYYY-MM-DD)
    IMDR_RESEARCH_UNTIL=2026-05-07   # latest publish date (YYYY-MM-DD)
    IMDR_RESEARCH_LIMIT=5       # cap on number of reports ingested
    IMDR_RESEARCH_PARALLEL=1    # leave at 1 — see "Concurrency" note above
"""
...
from ingest.crawler_goldman import ReportRef, discover_reports  # noqa: E402
    ...
    *,
    ...
    async with sem:
        meta = ReportMeta(
            vendor_code="goldman",
            vendor_id=0,
            title=ref.title or f"goldman/{ref.uuid}",
...
async def _amain() -> None:
    ...
    limit = _read_int_env("IMDR_RESEARCH_LIMIT", 0)  # 0 = no cap
    ...
    if settings.research_drop_single_name_equity:
        ...
        refs, _ = apply_relevance_filter(
            vendor_code="goldman", refs=refs, verbose=True,
        )
    if limit:
        refs = refs[:limit]
    ...
    for r in refs:
        ...
        print(f"    {r.publish_date}  {r.uuid[:8]}  {title}")
    print()
    ...
    for o in outcomes:
        if o.error is not None:
            failed += 1
            print(f"  [FAIL]   {o.ref.uuid[:8]}  {o.ref.title[:40]:<40}  {type(o.error).__name__}: {o.error}")
            continue
        r = o.result
        if r.was_inserted:
            inserted += 1
            print(
                f"  [INS]    id={r.report_id:<5} {o.ref.uuid[:8]}  "
            ...
            skipped += 1
            print(
                f"  [DUP]    id={r.report_id:<5} {o.ref.uuid[:8]}  "
