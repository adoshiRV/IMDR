"""Discover and ingest today's ANZ Research reports.

Walks ANZ Research hub pages with the persistent profile, dedupes
report tiles by ``docRef``, filters by publish_date, and ingests each
PDF end-to-end. ANZ Research runs on the SingleTrack CMS — see
``ingest/crawler_anz.py`` for the tile DOM structure and download URL
pattern.

Defaults
--------
* Date window: ``today and yesterday`` (UTC). ANZ publishes from APAC;
  the 2-day window catches early reports without much noise.
* Embed: OFF (set ``IMDR_RESEARCH_EMBED=true`` to enable).
* Concurrency: 1 PDF at a time (Chrome locks the persistent profile dir).

Usage
-----
    C:/Users/adoshi/.conda/envs/imdr/python.exe playground/research/ingest_today_anz.py

Optional env
------------
    IMDR_RESEARCH_EMBED=true              # enable embedding
    IMDR_RESEARCH_EMBED_MODEL=gemini-embedding-2   # model selection
    IMDR_RESEARCH_SINCE=2026-05-05        # earliest publish date
    IMDR_RESEARCH_UNTIL=2026-05-07        # latest publish date
...
from ingest.crawler_anz import ReportRef, discover_reports  # noqa: E402
...
async def _ingest_one_ref(
    ...
    *,
    ...
    async with sem:
        # Use ANZ publication_type as a soft asset_class hint when present.
        meta = ReportMeta(
...
async def _amain() -> None:
    ...
    for r in refs:
        ...
        print(f"    {r.publish_date}  {r.uuid[:8]}  {ptype:<22}  {title}")
    print()
    ...
    for o in outcomes:
        if o.error is not None:
            failed += 1
            print(
                f"  [FAIL]   {o.ref.uuid[:8]}  {o.ref.title[:40]:<40}  "
