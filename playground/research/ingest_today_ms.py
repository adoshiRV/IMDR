"""Discover and ingest today's Morgan Stanley Matrix research reports.

Walks ``/eqr/research/portal/home/global`` (the master cross-asset
feed), pulls per-report metadata via the frontmatter JSON API, and
ingests each PDF end-to-end. See ``ingest/crawler_ms.py`` for the
URL chain.
...
Usage
-----
    C:/Users/adoshi/.conda/envs/imdr/python.exe playground/research/ingest_today_ms.py
"""
...
from ingest.crawler_ms import ReportRef, discover_reports  # noqa: E402
...
async def _amain():
    ...
    for o in outcomes:
        ...
        if r.was_inserted:
            inserted += 1
            print(f"  [INS]    id={r.report_id:<5} {o.ref.uuid[:8]}  chunks={r.n_chunks:>3} embeds={r.n_embeddings:>3}  total={r.timings_s.get('total', 0):.1f}s  {o.ref.title[:40]}")
        else:
            skipped += 1
            print(f"  [DUP]    id={r.report_id:<5} {o.ref.uuid[:8]}  already in DB  {o.ref.title[:40]}")
    print("-" * 60)
