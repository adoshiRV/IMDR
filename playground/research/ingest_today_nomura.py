"""Discover and ingest today's Nomura Now research reports.

Walks Nomura's ``/research/m/Home`` listing, dedupes by numeric
``report_id``, filters by publish_date and PDF-availability, and
ingests each PDF end-to-end. Nomura is the cleanest fetch shape of
any vendor so far — the per-report download URL is deterministic
(``https://go.nomuranow.com/research/japi/publication/{id}.file``)
and serves PDF bytes directly with the persistent session cookies.

Defaults
--------
* Date window: ``today and yesterday`` (UTC).
* Embed: OFF (set ``IMDR_RESEARCH_EMBED=true`` to enable).
* Concurrency: 1 (Chrome locks profile dir).

Usage
-----
    C:/Users/adoshi/.conda/envs/imdr/python.exe playground/research/ingest_today_nomura.py
"""
...
from ingest.crawler_nomura import ReportRef, discover_reports  # noqa: E402
